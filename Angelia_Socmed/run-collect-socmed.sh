...
BASE_LOG_DIR=/data/cms/higt_reflex/angelia/socmed/script
...
SOURCE_DIR=/data/sftpuser/angelia/socmed_profile/

#-get collectedd files
${MYSQL_HOME} -h${MYSQL_HOST} -u${MYSQL_USER} -p${MYSQL_PASSWORD} -NB -e "SELECT CONCAT('${SOURCE_DIR}',FILENAME,'|',FILESIZE) FILENAME FROM TB_FILE_COLLECT_${TABLE_CONTROL} USE INDEX(SCP_ID) WHERE SOURCE_TYPE = '${SOURCE_NAME}' AND SCP_ID = '${SRC_TYPE}' AND FILEDATE = ${CURR_DATE} ORDER BY FILENAME" ${MYSQL_DB} > ${COLLECTED_FILE_LIST} 2>&1
...
FILE_DATE=`date -d ${CURR_DATE} +%Y-%m-%d`
...
if [ `cat ${UPL_LIST_FILE}|wc -l` -eq 0 ]; then
    ...
    cd ${SOURCE_DIR}; ls -l ${SRC_TYPE_LOWER}*${FILE_DATE}* > ${SRC_LIST_FILE}.all
    ...
    #cat ${SRC_LIST_FILE}.all |grep "${FILE_DATE}" |grep "control"|awk -v DIR=${SOURCE_DIR} '{print DIR$9}' |sort -u > ${CONTROL_LIST_FILE}
    #} &
        
    {
    #list file, in format: filename|filesize
    #cat ${SRC_LIST_FILE}.all |grep "${FILE_DATE}"|grep -v "control" |awk -v DIR=${SOURCE_DIR} '{print DIR$9"|"$5}' |sort -u > ${SRC_LIST_FILE}.tmp
    cat ${SRC_LIST_FILE}.all |grep "${FILE_DATE}"|grep -v "control" |awk -v DIR=${SOURCE_DIR} '{print DIR$9"|"$5}' |sort -u > ${SRC_LIST_FILE}
    } &
...
for srcFile in `cat ${UPL_LIST_FILE}`; do
    ...
    echo "
    DELETE FROM TB_FILE_COLLECT_${TABLE_CONTROL} WHERE SCP_ID = '${SRC_TYPE}' AND FILEDATE = ${CURR_DATE} AND FILENAME = '${FILE_NAME}';
    DELETE FROM TB_FILE_TO_JOB_${TABLE_CONTROL} WHERE SCP_ID = '${SRC_TYPE}' AND FILEDATE = ${CURR_DATE} AND REAL_FILENAME = '${baseFileName}' AND TRANF_JOB_ID = '0';
    INSERT INTO TB_FILE_COLLECT_${TABLE_CONTROL} (FILENAME,FILESIZE,TOTAL_RECORDS,SCP_ID,FILEDATE,SOURCE_DIR,SOURCE_TYPE,COLL_JOB_ID) VALUES ('${FILE_NAME}',${SOURCE_FILE_SIZE},'${TOTAL_FILE_ROW}','${SRC_TYPE}',${CURR_DATE},'${HFS_INPUT_DIR}','${SOURCE_NAME}','${JOB_ID}');
    INSERT INTO TB_FILE_TO_JOB_${TABLE_CONTROL} (REAL_FILENAME,ORIGINAL_FILESIZE,SCP_ID,FILEDATE,SOURCE_DIR,SOURCE_TYPE,COLL_JOB_ID,TRANF_JOB_ID) VALUES ('${baseFileName}',${SOURCE_FILE_SIZE},'${SRC_TYPE}',${CURR_DATE},'${HFS_INPUT_DIR}','${SOURCE_NAME}','${JOB_ID}','0');
    " >> ${LOG_FILE}.ins
